                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTBC ought to be enough for anybody." - Bill Gates, 2017
    ui->zTBCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTBCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTBC->setEnabled(false);
        ui->pushButtonMintzTBC->setToolTip(tr("zTBC is currently disabled due to maintenance."));

        ui->pushButtonSpendzTBC->setEnabled(false);
        ui->pushButtonSpendzTBC->setToolTip(tr("zTBC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTBCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTBC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTBC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zTBC, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTBC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTBC in ") +
...
void PrivacyDialog::on_pushButtonSpendzTBC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTBC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zTBC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zTBC
        sendzTBC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTBC
    sendzTBC();
}

void PrivacyDialog::on_pushButtonZTBCControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZTbcControlDialog* zTbcControl = new ZTbcControlDialog(this);
    zTbcControl->setModel(walletModel);
    zTbcControl->exec();
}

void PrivacyDialog::setZTbcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTBCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTBC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Tbc Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTBCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTBC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TBC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTBC</b>";
    ...
    // use mints from zTbc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTbcControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTbcControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zTbc was spent successfully update the addressbook with the label
    ...
    // Clear zTbc selector in case it was used
    ZTbcControlDialog::listSelectedMints.clear();
    ui->labelzTBCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTBC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TBC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTBC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strUnconfirmed = "";
        if (nUnconfirmed) {
            strUnconfirmed += QString::number(nUnconfirmed) + tr(" unconf. ");
        }
        if(nImmature) {
            strUnconfirmed += QString::number(nImmature) + tr(" immature ");
        }
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTBC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTBC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTBC "));
    ui->labelzTBCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zTBC </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zTBC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzTBCSyncStatus->setVisible(fShow);
}
