...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TBC);
    ...
    if (!settings.contains("nAnonymizeTbcAmount"))
        settings.setValue("nAnonymizeTbcAmount", 1000);

    nAnonymizeTbcAmount = settings.value("nAnonymizeTbcAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTbcAmount"))
        SoftSetArg("-anonymizetbcamount", settings.value("nAnonymizeTbcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tbc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTbcAmount:
            return QVariant(nAnonymizeTbcAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTbcAmount:
            nAnonymizeTbcAmount = value.toInt();
            settings.setValue("nAnonymizeTbcAmount", nAnonymizeTbcAmount);
            emit anonymizeTbcAmountChanged(nAnonymizeTbcAmount);
            break;
        case CoinControlFeatures:
