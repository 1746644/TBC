...
const std::string VERSION_CHECK_SERVER = "www.tbc.kim";
const std::string VERSION_CHECK_PATH   = "/software/version.txt";

bool httpGet(/*out*/string& result, const string& host, const string& url, boost::asio::io_service &_io)
{
    try
    {
        string domain = host;
        boost::asio::ip::tcp::resolver resolver(_io);
        boost::asio::ip::tcp::resolver::query query(domain, "http");
        ...
        {
            if (fDebug)
                LogPrintf("Check new version wallet error : Connect to server failed\n");
            return false;
        }
        ...
        boost::asio::streambuf response(4096);
        ...
        {
            if (fDebug)
                LogPrintf("Check new version wallet error : [Get Address] - Invalid response\n");
            return false;
        }
        if (status_code != 200)
        {
			if (fDebug)
                LogPrintf("Check new version wallet error : [Get Address] - Response returned with status code %d\n", status_code);
            return false;
        }
        ...
        {
            if (fDebug)
                LogPrintf("Check new version wallet error : Get response data failed\n");
			return false;
        }
    ...
    {
		if (fDebug)
			LogPrintf("Check new version wallet exception : %s\n", e.what());
	}

    return false;
}

std::string getCleanVersionString(std::string fullVersion)
{
    std::string result = fullVersion;
    ...
    if(std::string::npos != found)
        result.erase(found, result.length() - found);
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("TBC Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::checkLatestWalletVersion()
{
    std::string currentVersion = getCleanVersionString(FormatFullVersion());
    if (fDebug)
        LogPrintf("Current wallet version : %s\n", currentVersion);

    boost::asio::io_service io_service;
    std::string lastVersion = "";
    if(httpGet(lastVersion, VERSION_CHECK_SERVER, VERSION_CHECK_PATH, io_service))
    {
        if(lastVersion.length() > 0)
        {
            lastWalletVersion = getCleanVersionString(lastVersion);
            LogPrintf("Latest wallet version : %s\n", lastWalletVersion);

            if(currentVersion.compare(lastWalletVersion) < 0)
            {
                QMessageBox::information(this, tr("Upgrade"),
										 tr("New version wallet released, please download from official website and upgrade."));
            }
        }
    }
}
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a TBC address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and TBC: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zTBC"));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About TBC Core"), this);
    aboutAction->setStatusTip(tr("Show information about TBC Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for TBC"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your TBC addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TBC addresses"));
    ...
    openAction->setStatusTip(tr("Open a TBC: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the TBC Core help message to get a list with possible TBC command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("TBC Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TBC network", "", count));
}
...
{
    QString strTitle = tr("TBC Core"); // default title
