...
#include "ui_ztbccontroldialog.h"
...
std::list<std::string> ZTbcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTbcControlDialog::listMints;

ZTbcControlDialog::ZTbcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTbcControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTbcControlDialog::~ZTbcControlDialog()
{
    delete ui;
}

void ZTbcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTbcControlDialog::updateList()
{
...
void ZTbcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTbcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTbc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTbcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTbcControlDialog::GetSelectedMints()
{
...
void ZTbcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
