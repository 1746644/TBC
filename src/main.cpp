...
#if defined(NDEBUG)
#error "TBC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utbc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zTBC with serial %s is already in the block %d\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.ContainsZerocoins() && chainActive.Height() < Params().Zerocoin_StartHeight())
        return state.DoS(100, error("AcceptToMemoryPool: : CheckTransaction failed"), REJECT_INVALID, "zerocoin-not-allowed");    
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTBC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zTBC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        ...
        } else if (nHeight < 360 && nHeight > 0) {
            nSubsidy = 1 * COIN;            
        } else if (nHeight < Params().LAST_POW_BLOCK() && nHeight >= 360) {
            nSubsidy = 100 * COIN;
        ...
        // Super block
        int nTail = nHeight - nHeight / 2160 * 2160;
        if (nHeight > Params().LAST_POW_BLOCK() && nTail >= 0 && nTail < 3) {
            nSubsidy = 1000 * COIN;
        }
...
int64_t GetMasternodePaymentA(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TBC
...
{
    RenameThread("tbc-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTBCMinted()
{
...
void RecalculateZTBCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTBC supply
...
bool RecalculateTBCSupply(int nHeightStart)
{
...
bool UpdateZTBCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTBCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zTBC money supply in the block index
    if (!UpdateZTBCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTBC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTbcSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TBC to zTBC
    if (pwalletMain->isZeromintEnabled() && chainActive.Height() >= Params().Zerocoin_StartHeight())
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Check premine script
    if (nHeight == 1) {
        CScript scriptPubKey = CScript() << Params().PreminePubKey() << OP_CHECKSIG;
        if (block.vtx[0].vout[0].scriptPubKey != scriptPubKey) {
            return error("CheckBlock() : CheckPremineTransaction failed");            
        }
    }
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTBC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTBC serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (isBlockFromFork) {
            ...
            // Inputs
            std::vector<CTxIn> tbcInputs;

            for (CTxIn stakeIn : stakeTxIn.vin) {
                tbcInputs.push_back(stakeIn);
            }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTBC mints and %d zTBC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TBC: We use certain sporks during IBD, so check to see if they are
        ...
        BOOST_FOREACH (CAddress& addr, vAddr) {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable()) {
                // Relay to a limited number of other nodes
                {
                    ...
                    if (!lockNodes) return error("lock cs_vNodes failed");
...
{
    {
        ...
        TRY_LOCK(cs_vNodes, lockNodes);
        if (!IsInitialBlockDownload() && (GetTime() - nLastRebroadcast > 24 * 60 * 60) && lockNodes) {
            BOOST_FOREACH (CNode* pnode, vNodes) {
