...
CMasternode* CMasternodeMan::GetCurrentMasterNode(int mod, int64_t nBlockHeight, int minProtocol)
{
 ...
 std::map<int64_t,CMasternode> CMasternodeMan::GetCurrentMasterNodeV(int mod, int64_t nBlockHeight, int minProtocol)
{
    int64_t score = 0;//now max big
    int64_t scoremin = 0;//now max big
    //CMasternode* winner = NULL;
    std::map<int64_t,CMasternode> vwinners;
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        else
        {
            if(n2>scoremin)
            {
                std::map<int64_t,CMasternode>::iterator i=vwinners.find(scoremin);
                if(i!=vwinners.end())
                {
                        vwinners.erase(i);
                ...
                for(itt=vwinners.begin();itt!=vwinners.end();itt++)
                {
                    if(itt->first<scoremin)
                    {
                        scoremin=itt->first;
                    }
                }
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 TBC tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 TBC tx -> 1 confirmation
