...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TBC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTBC send
enum ZerocoinSpendStatus {
    ZTBC_SPEND_OKAY = 0,                            // No error
    ZTBC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTBC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTBC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTBC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTBC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTBC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTBC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTBC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTBC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTBC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTBC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTBC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTBC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTBC_SPENT_USED_ZTBC = 14,                      // Coin has already been spend
    ZTBC_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZTbcAutoBackups(bool fEnabled)
    {
    ...
    std::map<libzerocoin::CoinDenomination, CAmount> GetMyZerocoinDistribution(bool isMatured = true) const;
    ...
    /** zTBC reset */
    boost::signals2::signal<void()> NotifyzTBCReset;
};
