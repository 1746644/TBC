...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    vector<pair<int64_t, uint256> > vSortedByTimestamp;
    vSortedByTimestamp.reserve(64 * getIntervalVersion() / Params().TargetSpacing());
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval) {
        if (!pindexNext) {
            // reconfirm because pindexNext often be Null
            pindexNext = chainActive.Next(pindex);
            if (!pindexNext) {
                // Should never happen
        ...
        pindex = pindexNext;
        pindexNext = chainActive.Next(pindexNext);
        if (pindex->GeneratedStakeModifier()) {
...
{
    //TBC will hash in the transaction hash and the index number in order to make sure each hash is unique
