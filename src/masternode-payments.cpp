...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CScript payee;
     std::map<int64_t,CMasternode> winningNodeMap;
     int mapsize=0;
    //spork
    ...
    if (true) {
        ...
        //CMasternode* winningNode = mnodeman.GetCurrentMasterNode(1);
         winningNodeMap= mnodeman.GetCurrentMasterNodeV(1);
         mapsize=winningNodeMap.size();
        if (mapsize!=0) {
    ...
    else
    {
            ...
            if (hasPayment && masternodePayment > 0) {
                if (fProofOfStake) {
                    ...
                    LogPrintf("FillBlockPayee:  txNew.vout.size():[%d]\n",i);
                ...
                LogPrint("masternode"," spork Masternode payment of %s to %s\n", FormatMoney(masternodePayment).c_str(), address2.ToString().c_str());
            } else {
                //miners get the full amount on these blocks
                txNew.vout.resize(1);
                txNew.vout[0].nValue = blockValue;
                LogPrint("masternode"," spork Miners get the full amount %s\n", FormatMoney(blockValue).c_str());
            }
    ...
    CAmount blockValue = GetBlockValue(pindexPrev->nHeight);
    CAmount masternodePayment = GetMasternodePaymentA(pindexPrev->nHeight, blockValue,mapsize);
    ...
    if (hasPayment && masternodePayment > 0) {
        if (fProofOfStake) {
            ...
            unsigned int i = txNew.vout.size();
            txNew.vout.resize(i + mapsize);

            txNew.vout[i - 1].nValue -= (masternodePayment*mapsize);

            std::map<int64_t,CMasternode>::iterator itt;
            for(itt=winningNodeMap.begin();itt!=winningNodeMap.end();itt++)
            {
                payee = GetScriptForDestination(itt->second.pubKeyCollateralAddress.GetID());
                txNew.vout[i].scriptPubKey = payee;
                txNew.vout[i].nValue = masternodePayment;
                i++;
                CTxDestination address1;
            ...
            txNew.vout.resize(mapsize+1);
            txNew.vout[0].nValue = blockValue - (masternodePayment*mapsize);
            int index=1;
             std::map<int64_t,CMasternode>::iterator itt;
            for(itt=winningNodeMap.begin();itt!=winningNodeMap.end();itt++)
            {
                payee = GetScriptForDestination(itt->second.pubKeyCollateralAddress.GetID());
                txNew.vout[index].scriptPubKey = payee;
                txNew.vout[index].nValue = masternodePayment;
                index++;
        ...
        txNew.vout.resize(1);
        txNew.vout[0].nValue = blockValue;
        LogPrint("masternode","Miners get the full amount %s\n", FormatMoney(blockValue).c_str());
    }
}
