    ...
    if (wtx.IsCoinStake()) {
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecordEx sub(hash, nTime);
                sub.type = TransactionRecordEx::ZerocoinSpend_Change_zTbc;
...
std::string formatTxType(const TransactionRecordEx::Type type)
{
    switch (type) {
		...
		case TransactionRecordEx::ZerocoinMint:
			return std::string("Converted TBC to zTBC");
		case TransactionRecordEx::ZerocoinSpend:
			return std::string("Spent zTBC");
		case TransactionRecordEx::RecvFromZerocoinSpend:
			return std::string("Received TBC from zTBC");
		case TransactionRecordEx::ZerocoinSpend_Change_zTbc:
			return std::string("Minted Change as zTBC from zTBC Spend");
		case TransactionRecordEx::ZerocoinSpend_FromMe:
			return std::string("Converted zTBC to TBC");
